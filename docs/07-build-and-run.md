...
| Upload fails with "no upload port" | USB cable is charge-only; or the board's USB-UART driver is not installed | Use a known-good USB-data cable; install the Silicon Labs CP210x driver (or CH340 depending on the board variant). |
| Wi-Fi `ESP32-Console` not visible | The board didn't boot, or the laptop's Wi-Fi adapter doesn't pick up 2.4 GHz at low signal | Reset the board (EN button); move within ~1 m of the board; verify the boot lines in the serial monitor. |
| GUI shows "timed out during opening handshake" | Laptop is not on the `ESP32-Console` network, or WSL is not in mirrored networking mode | Connect Windows to the ESP AP; check `ip addr` in WSL shows a `192.168.4.x` interface. |
| Tests fail with `libEGL.so.1: cannot open shared object file` | Qt runtime libraries missing in CI environment | Already handled in CI; locally, install `libegl1` and friends. |
